  ...
  constructor(
    ...
    const crossplaneIamRole = new IamRoleForKubernetesSA(
      ...
      `${id}-crossplane-aws-iam-role`,
      {
        ...
        // Crossplane appends a hash suffix to the SA name at runtime (e.g. provider-aws-iam-08becec3bc41).
        // StringLike + wildcard ensures AssumeRoleWithWebIdentity succeeds regardless of the suffix.
        serviceAccountName: `${SERVICE_ACCOUNTS.crossplaneAwsProvider}*`,
        assumeRoleConditionTest: 'StringLike',
      }
    );
    ...
    // ── Pre-destroy cleanup ────────────────────────────────────────────────
    // Runs before fluxConfig/flux/eks/vpc are destroyed.
    // Handles resources outside Terraform state: Crossplane IAM objects and
    // the Istio NLB, which would otherwise orphan AWS resources or cause
    // VPC subnet deletion to fail with DependencyViolation.
    //
    // Destroy order (via reverse of addDependency):
    //   cleanup → fluxConfig → flux → eks → vpc
    const cleanup = new CleanupStack(this, `${id}-cleanup`, {
      clusterName: CORE_CLUSTER_NAME,
      region: secrets.aws.region,
    });
    cleanup.addDependency(fluxConfig);
